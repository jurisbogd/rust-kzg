...
unsafe fn deserialize_blob(blob: *const Blob) -> Result<Vec<ZFr>, C_KZG_RET> {
    (*blob)
        ...
        .map(|chunk| {
            ...
            if let Ok(result) = ZFr::from_be_bytes(bytes) {
                Ok(result)
            } else {
...
pub unsafe extern "C" fn verify_kzg_proof(
    ...
    let frz = handle_ckzg_badargs!(ZFr::from_be_bytes((*z_bytes).bytes));
    let fry = handle_ckzg_badargs!(ZFr::from_be_bytes((*y_bytes).bytes));
...
pub unsafe extern "C" fn compute_kzg_proof(
    ...
    let frz = match ZFr::from_be_bytes((*z_bytes).bytes) {
        Ok(value) => value,
        Err(_) => return C_KZG_RET_BADARGS,
    };
